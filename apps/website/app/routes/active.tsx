import type { Route } from "./+types/active";
...
import { getActiveUSFundData, type OTCFundData } from "~/lib/market-data";
...
export async function loader() {
  // 流式返回：标题计数/筛选标签的 count 依赖 funds，需要等数据进来；筛选按钮 + 表格 + 空状态也依赖 funds。
  return {
...
export default function Active(_args: Route.ComponentProps) {
  const { funds, fetchedAt } = useLoaderData<typeof loader>();
  return (
    <div className="min-h-screen bg-background">
      <AppHeader currentLabel="美股主动" />
...
function ActiveContent({
  ...
  const toggleSort = (field: SortField) => {
    if (sortField === field) {
      ...
      setSortField(field);
      setSortDir(
        field === "returnOneYear" || field === "scale" || field === "returnSinceInception"
          ? "desc"
          : "asc",
      );
    }
  ...
  // 不使用 useMemo（funds 列表数量有限，filter+sort 计算量可忽略，避免 hooks 顺序问题）
  const filtered = (() => {
    let list = [...funds];
  ...
  return (
    <>
      ...
      <FadeIn className="mb-6 flex items-end justify-between" delay={0.1}>
        <div>
          <h1 className="mb-2 text-xl font-bold tracking-tight md:text-2xl">{title}</h1>
